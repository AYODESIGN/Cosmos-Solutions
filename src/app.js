...
setInterval(() => {
  document.title = alertShow ? 'SOLUTION COSMOS 😍' : 'SOLUTION COSMOS ✌';
...
function showModal() {
  ...
  cartModal.style.top = '50px'; // Adjust this value as needed to position the modal closer to the top
  cartModal.style.left = '50%'; // Center the modal horizontally
  cartModal.style.transform = 'translateX(-50%)'; // Center the modal horizontally
  cartModal.style.overflowY = 'auto'; // Allow vertical scrolling if content exceeds modal height
  const modalTop = cartModal.getBoundingClientRect().top + window.scrollY;
  window.scrollTo({
    top: modalTop,
